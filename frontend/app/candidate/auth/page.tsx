...
import { Sparkles, Mail, Lock, User, ArrowRight, ShieldCheck, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react';
...
export default function CandidateAuthPage() {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.requestOTP(email, 'candidate');
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.verifyOTP(email, otpCode, 'candidate', fullName);
      const data = res.data;

      // Persist session
      localStorage.setItem('ecosphere_token', data.token);
      localStorage.setItem('ecosphere_user_id', data.user_id);
      localStorage.setItem('ecosphere_candidate_id', data.candidate_id || '');
      localStorage.setItem('ecosphere_email', data.email);
      localStorage.setItem('ecosphere_role', 'candidate');
      localStorage.setItem('ecosphere_name', data.full_name || '');
    ...
    <div className="flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50 p-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Candidate Portal</h1>
          <p className="text-sm text-slate-500 mt-1">Sign in to take AI mock interviews and track evaluations</p>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="123456"
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-lg font-mono tracking-widest text-center text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all"
                />
              </div>
              <p className="text-[11px] text-slate-400 mt-1">
                Enter the OTP sent to <b>{email}</b>. (Dev fallback: <code>123456</code> or dev code above)
              </p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            onClick={() => window.location.href = "http://localhost:8000/api/auth/google/login?role=candidate"}
          >
          ...
          <button
            type="button"
            className="flex items-center justify-center gap-2 py-2.5 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors text-sm font-semibold text-slate-700"
            onClick={() => window.location.href = "http://localhost:8000/api/auth/github/login?role=candidate"}
          >
        ...
        {/* Footer switch to organization */}
        <div className="mt-8 pt-6 border-t border-slate-100 text-center">
          <p className="text-xs text-slate-500">
            Hiring manager or recruiter?{' '}
            <Link href="/organization/auth" className="text-blue-600 font-semibold hover:underline">
              Organization Login
            </Link>
          </p>
        </div>
